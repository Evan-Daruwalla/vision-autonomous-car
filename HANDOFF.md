...
**Last updated: 2026-09-03 ~22:44 CDT** — this file is the ONLY live snapshot.
History lives in `docs/Project Record — Full Chronological History.md`
(append-only, 110 appendices A–DF; BM/BN/BP/BU/CT/CY/DB/DE are corrections to earlier entries). When this file and the record disagree about
...
| 3c | ~~**SERVO-TO-PINION COUPLING — part not yet named.**~~ **CHOSEN 2026-09-03: Adafruit #4252** (Appendix DF), already built, Evan reports "strong" — empirically confirms the spline fit on his MG90S. **PRD task 8c(ii) still not fully closed**: no numeric stall-torque margin exists or can be computed (injection-moulded, no published torque rating) — Evan's field report is the ceiling of what's known without a destructive test. | — |
...
| 5 | **Place the order** (`docs/BOM.md`, **$402.45 + shipping ≈ $415-430** — stale $238-248 figure corrected here). Nothing downstream of M1.5 moves until parts exist. | **Evan** |
...
| Track layout | M1.4 | **v1 + v2 drawn, NOT committed** | 2026-09-01, Appendices AM/AT/AX/AY. `cad/track_layout_v1.py` = figure-8 with a flat causeway bridge + 5 landmarks; `cad/track_layout_v2.py` = 3x3 city grid, 9 intersections, figure-8 route through the centre, 8.31 m, **103.5 mm spare at R=500** (re-run 2026-09-03 on the new 148.25 mm rear track: lane 296.5 mm, span 2696.5 mm — the tightest of the three widths and less than the original 130 mm estimate allowed). Both parametric and self-checking; `--car-width` flag added. **Corner geometry FROZEN until the T2 turning test** |
...
| Uno firmware | — | **CONTROL FIRMWARE RUNS ON THE BOARD (2026-09-02)** | Appendices BD/BJ/BO. `firmware/` — `uno_bringup` (board proven), `uno_memtest` (2048 B SRAM, 16.0042 MHz), `uno_echo` (link p99 1.069 ms), `uno_packguard` (SELFTEST 27/27), **`uno_control` (implements `SERIAL_PROTOCOL.md` v0.2: **SELFTEST 49/49, `host_test.py` 13/13 exit 0**, 7888 B flash / 313 B SRAM, loop 2-3 ms; the status LED is quiet on a bench board and loud once a real pack has ever been seen — Appendix CE)** — **CURRENTLY FLASHED, fixed build, as of 2026-09-02 ~19:55 CDT.** Evan had put `uno_packguard` back on it in between, so **read the boot banner, never this line, for what the board is running.** ⚠️ **Watchdog defect found by the daily-audit (BY), fixed (CA): two `millis()` samples per loop iteration made the watchdog trip in the same iteration as every good frame — after the reply was sent, so no status byte ever showed it; a wired car would drive ~2 ms then brake ~48 ms, repeating.** The probe that catches it reads `armed` inside the window; the status-bit check the audit suggested passes on the broken build. `SERIAL_PROTOCOL.md` is no longer design-only. **ACTUATORS UNWIRED** — link + state machine only. ⚠️ **The repo source no longer matches what's flashed, as of 2026-09-03 (Appendix DC): `uno_control.ino`'s lighting path was rewritten for WS2812B (task 8e) after this row's "CURRENTLY FLASHED" build. That rewrite is compiled, not flashed** — the board still runs the 2026-09-02 build described in the rest of this row; SELFTEST 49/49 and 7888 B flash refer to THAT build, not the current source (58 checks, 9680 B, per Appendix DC) |
...
- **THE ORDER (`docs/BOM.md`, $402.45 + shipping = ≈$415-430 — after the Vilros kit, Amazon consolidation, right-sizing bulk items, swapping 8 discrete LEDs for a WS2812B strip, Evan choosing the 144 LED/m density over 60/m (+$4.00), and choosing the Adafruit #4252 steering coupling over the free-but-rejected printed option (+$0.75, Appendix DF)).** Nothing is bought. **Lighting**: WS2812B strip, 2 segments on **D4 and D7** (data is one-way, cut halves cannot share a feed), freeing **D5/D6 — the first spare PWM this design has had**. Encoder-interrupt risk quantified at ~0.027% tick loss for 3 px/segment (Appendix CZ); it scales with pixels per segment, so **cut to pixel count, not length**. **`docs/WIRING_PROTOSHIELD.md` and `firmware/SERIAL_PROTOCOL.md` are now current (Appendix DA)** — the vet found §2.2 naming the wrong motor (#1093) and no strip pins assigned at all. ~~⚠️ **`docs/LIGHTING_SPEC.md` still describes the superseded 8-LED scheme.**~~ **CLOSED same day (Appendix DC): rewritten for WS2812B, 3px/segment, DRL folded into the head pixel only (Evan's calls).** `firmware/uno_control/uno_control.ino`'s `applyLights()` is rewritten too — compiled clean (9680 B flash / 371 B SRAM via `arduino-cli`), **not hardware-verified**, no strip owned. PRD task 8e is now blocked on hardware only, not decisions.
