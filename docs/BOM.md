...
| 8 | USB power bank, **5V/3A** | → Pi ONLY. ✅ **CONFIRMED 2026-09-05 (Evan): label reads 5V/3A, 10,000 mAh** — closes Verify item 1 below. 10,000 mAh matches `docs/research/2026-07-23_power-system.md`'s own working figure (Battery A, ~4.3h Pi-only runtime) exactly — that runtime estimate was an assumption until now, confirmed against the real hardware | **$0.00** | owned |
...
| 22 | **Servo-to-pinion coupling — CHOSEN: Adafruit #4252** | ⚠️ **HARD CONSTRAINT, not a preference** (Appendices BS/BV). **The steering coupler is the highest-torque joint on the car and a PRINTED cross-axle stub FAILS there: SF 0.57–0.96** at MG90S stall, against SF 2.26–3.77 for the drive coupler it was modelled on — must grip a real Lego axle, never a printed cross profile. **Evan CHOSE the Adafruit #4252 (2026-09-03, Appendix DF), already built, reports it "strong."** That empirically confirms the spline fit on his real MG90S — Adafruit's own listing would not guarantee it beyond their own micro servo, and this closes that uncertainty by build rather than by spec. ⚠️ **No numeric stall-torque margin exists for this part and none can be computed** — it is injection-moulded plastic, not PLA, so Appendix BV's PLA interlayer-shear math does not transfer, and Adafruit publishes no torque rating. Evan's field report is the ceiling of what's known without a destructive test; treated as sufficient for a hobby-scale build, not as a stated SF. Printed alternatives (Printables 61922/147626) remain rejected. ⚠️ No Amazon/eBay listing of this specific adapter exists — Adafruit is the sole source, **re-confirmed 2026-09-03 (Appendix DG) across Amazon, eBay AND AliExpress** when Evan objected to ~$10 shipping for a $0.75 part; Adafruit has no free-shipping threshold and consolidating other rows onto them makes shipping worse, so there is no bundling escape either. **A printed stopgap now exists** — `cad/servo_lego_coupler_v1.stl`, a socket-style design that prints no cross profile (obeys Appendix BV) — but it has **no torque margin either**, so this row stays until one of them survives a real test | **$0.75** | [adafruit.com/product/4252](https://www.adafruit.com/product/4252) |
...
| | | **TOTAL (sum of rows, current prices)** | **$402.45** | |
...
**Third pass, same day: the 3 discrete-LED packs are gone entirely, not just resized.** Evan proposed WS2812B addressable RGB strips instead of 8 discrete 3mm LEDs — "much easier to wire and mount." Verified: it is, and it also frees 2 of the car's last 3 PWM pins (WS2812B is software-timed, needs no PWM hardware regardless of pin count). ⚠️ **Corrected same day (Evan): "the led strip only moves data 1 way"** — DIN→DOUT only, so the 2 cut segments each need their own data pin (2 pins, not 1), unless daisy-chained with an explicit jumper; 2 independent pins is simpler and avoids a DIN/DOUT mix-up, still frees the same 2 PWM pins either way. See row 18. Checked LM2596 headroom first (module rated 3A per Addicore's listing, closing a gap `WIRING_PROTOSHIELD.md` had flagged as unverified; existing peak load 840mA leaves ~2.16A margin — a strip at realistic brightness draws well under that). Picked **BTF-LIGHTING WS2812B, 1m/60px, $7.99, UL Listed, In Stock** — one reel, cut into 2 segments, not two separate reels. Drops the 3 LED packs ($20.97) and the 8-LED-resistor need (row 19), **net −$12.98**. **The two open figures (current draw, encoder tick loss) are now quantified, not just flagged** — see row 18. Both are small under stated assumptions (~80mA realistic; ~0.027% tick loss worst case) but depend on a pixel count and update strategy that aren't finalized, and neither is verified on real hardware yet. `docs/LIGHTING_SPEC.md` and `docs/WIRING_PROTOSHIELD.md` both still describe the old discrete-LED scheme and need updating to match — flagged, not done here.
...
**SUM OF ROWS: $402.45** before shipping. *(Was $401.70 until row 22's coupling was chosen 2026-09-03, Appendix DF — that $401.70 had used row 22's low end, $0.00, the free printed option Appendix BV already rejected; the real choice is the $0.75 Adafruit #4252, so the true sum is $401.70 + $0.75. Before that, was $397.70 — briefly under $400 — until Evan chose the 144 LED/m strip over the 60 LED/m one, +$4.00 for finer cut spacing.)* **A true sum of the row table** as of the landing-check sweep 2026-09-03: it had been $9.82 off, because row 13 was never folded onto Amazon (+$10.89) and row 16's subtotal never followed the 22 AWG revert (−$1.07). Both rows fixed; the figure and the rows now agree. ~~Every row in the BOM has a live-verified price~~ — **not true and never was**: rows 10 and 12 say "price NOT re-verified" (both Addicore, both backordered), row 19 says "not individually re-verified live". **Row 22 is now a point value** ($0.75, Adafruit #4252) — no longer the range this note used to describe. With shipping now concentrated mostly on ONE Amazon order (free with Prime or over $35, easily cleared) plus Vilros/Pololu/IMR/Adeept/DigiKey separately:
...
1. ~~**Your power bank's output rating**~~ ✅ **CONFIRMED 2026-09-05 (Evan): 5V/3A, 10,000 mAh.**
   Measured Pi 5 draw under CNN inference is 1.40A, comfortably under 3A.
   10,000 mAh matches the power research's own working assumption exactly
   (`docs/research/2026-07-23_power-system.md`, ~4.3h Pi-only runtime) —
   that estimate is now confirmed against real hardware, not assumed.
