...
| | | **TOTAL (sum of rows, current prices)** | **$415.34 – $425.34** | |
...
**Note on the last five Amazon-row items (22/26 AWG wire, heat shrink assortment, 0.1" headers, Dupont jumpers, USB-A-to-B cable):** real, in-category listings exist and were found (e.g. a 6-color 22 AWG spool set, a 720-piece heat-shrink assortment, a 40-pin breakaway header pack, a 120-piece Dupont kit, an Amazon Basics USB-A-to-B printer cable), each in the **$6–$12 range** based on comparable listings — but unlike everything else in this row, their exact current price/stock was **not individually re-verified via a live page load this session**, so no single link is pinned here. They are true commodities (any listing in spec works, matching this row's original "any" tag) and should be added to the same Amazon cart before checkout so they ship free with everything above. Budget **~$35–45** for all five together, which is folded into the estimate below.

⚠️ **CONSOLIDATION IS NOT FREE, AND THE JUMP IS LARGE — stated plainly rather than buried in a table.** Folding the small-order items and the generic parts onto Amazon (2026-09-03) cost **+$104.86 to +$108.61** over the previous piecemeal estimate, almost entirely because Amazon's hobby sellers sell in bulk packs, not the 1x quantities this build actually needs: **1 of 10** rocker switches, **1 of 20** XT30 pairs, **8 of 300** LEDs, **~20 of 600** resistors, **1 of 10** fuse holders. This is the same pattern as the ACEIRMC BMS (Appendix CO) — the individual per-unit price is often *cheaper* than the old single-unit sources — but repeated across ~10 items the cumulative overshoot is real money, not a rounding error. **This is a genuine tradeoff (fewer vendors, no more sub-shipping-cost line items, no more CAD currency problem — for roughly $105 more), and it has not been decided for Evan.**

**SUM OF ROWS: $415.34 – $425.34** before shipping (up from $310.48–$316.73). With shipping now concentrated mostly on ONE Amazon order (free with Prime or over $35, easily cleared) plus Vilros/Pololu/IMR/Adeept/DigiKey separately:
**≈$430 – $450 all-in** — a rough estimate, since per-vendor shipping was not re-summed line by line after this consolidation.
